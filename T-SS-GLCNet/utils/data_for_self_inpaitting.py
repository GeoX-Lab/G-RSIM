...
def random_resized_crop(image,size=(224,224), scale=(0.25, 0.75), ratio=(3./4., 4./3.)):
...
def join_transform(image,  input_size=(224,224)):
   ...
   # print(image.shape)
    image = random_resized_crop(image,size=(224,224),scale=(0.4,1.0))
...
class SegData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            if self.need_crop(image.shape):
                image, lbl = random_crop_pair(image, lbl, self.insize)
            # cv2.imshow('RGB', image[:, :, :3])
            # cv2.imshow('NIR', image[:, :, 3])
            # cv2.waitKey(0)
