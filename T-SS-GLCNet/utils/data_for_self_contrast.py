...
def join_transform(image,  input_size=(224,224)):
    """
...
def join_transform_v1(image,  input_size=(224,224)):
   ...
   # print(image.shape)
    image,label1 = random_resized_crop(image,label1,size=(224,224),scale=(0.4,1.0))
    image1,label2 = random_resized_crop(image1,label2,size=(224,224),scale=(0.4,1.0))
    band_num = image.shape[2]
    # Random Flip
    f = [1, 0, -1, 2, 2][np.random.randint(0, 5)]  # [1, 0, -1, 2, 2]
    if f != 2:
...
class SegData(data.Dataset):
    ...
    def __init__(
            ...
            opt,  # 包含了数据集路径
            ratio=1,
            transform=None,quick_train=True):
        self.opt = opt
...
#
        ...
        self.imgs =filelist_fromtxt(self.root + '/%s_%s' % (split1, self.dtype),self.root + '/%s_%s.txt' % (split, self.dtype), ifPath=True,ifarray=quick_train)# filelist(self.root + '/%s_%s' % (split, dt), ifPath=True)
...
class Train_Dataset(SegData):
    ...
    def __getitem__(self, index):
        ...
        '''
        h, w = self.imgs[self.dtype[0]][index].shape[:2]
        image = np.zeros((h, w, self.band), dtype=np.uint8)
        bs = 0  # band_start
        for dt in self.dtype:
            be = bs + Type2Band[dt]
            if dt in self.bl_dtype:
                bs = be  # Block some types of data(bands) when predicting
                continue
            img = self.imgs[dt][index]
            if Type2Band[dt] == 1:
        ...
        image,image1,lbl1,lbl2=join_transform(image)
        ...
        lbl1 = torch.from_numpy(np.squeeze(lbl1.astype(np.float32)).copy()).long()
        lbl2 = torch.from_numpy(np.squeeze(lbl2.astype(np.float32)).copy()).long()
        #for dt in self.dtype:
