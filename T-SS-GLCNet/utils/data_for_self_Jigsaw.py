...
def join_transform(image, input_size):
    """
    ...
    image = random_crop_pair(image,  input_size)
    image = randomShiftScaleRotate(
        image, shift_limit=(-0.1, 0.1), scale_limit=(-0.1, 0.1),
...
def join_transform_patch(patch,size=(75,75)):

    patch = random_crop_pair(patch,  (64,64))
    if len(patch.shape) > 2:
        height, width, channel = patch.shape
    else:  # TODO: test
        (height, width), channel = patch.shape, 1
    out_img = np.zeros((size[0], size[1], channel), dtype=np.uint8)

    if channel > 3:
        for i in range(channel):
            out_img[:, :, i] = cv2.resize(patch[:, :, i], (75,75))
    else:
        out_img = cv2.resize(patch, (75,75))
...
class SegData(data.Dataset):
    ...
    def __init__(
        ...
        #self.permutations = self.__retrive_permutations(9)
        '''
        ...
        if puzzle == 2:
            ...
            self.cw = [0, 75]
            #resize = (155, 155)
            self.crop_size = (150, 150)
            self.permutations = np.array(list(itertools.permutations(list(range(4)), 4)))
        elif puzzle == 3:
            self.cr = [0, 75, 150]
            self.cw = [0, 75, 150]
            #resize = (230, 230)
            self.crop_size = (225, 225)
            #print('permutations_hamming_max_%d.npy' % opt.num_classes)
            self.permutations = self.__retrive_permutations(1000)#np.array(list(itertools.permutations(list(range(9)))))
        ...
        self.names=filelist_fromtxt(self.root + '/%s_%s' % (split1, self.dtype),self.root + '/%s_%s.txt' % (split,  self.dtype),ifPath=True,ifarray=False)
        #self.lbls = filelist_fromtxt(self.root + '/' + split1 + '_lbl', self.root + '/' + split + '_lbl.txt',ifPath=True,ifarray=True)
        ...
        print('Actual number of samples used = %d' % int(len(self.imgs) * self.ratio))
...
class Train_Dataset(SegData):
    ...
    def __getitem__(self, index):
       ...
       # if len(lbl.shape)==3:
        ...
        image = join_transform(image,  self.crop_size)
        ...
        for cr in self.cr:
            for cw in self.cw:
                patch = image[cr: cr+75, cw: cw+75, :].copy()
                patch=join_transform_patch(patch)
                patch =torch.from_numpy( patch.astype(np.float32).transpose(2, 0, 1).copy() / 255.0 * 3.2 - 1.6)
        ...
        data_dict['image'], data_dict['label'], data_dict['patchs'] = data, np.array(lbl, dtype=np.int64),patchs
