""" This main entrance of fine-tuning for segmentation donwstream tasks
"""
import os
...
def main(args, stream):
    ...
    global_bs = args.num_gpus * args.batch_size if args.num_gpus > 1 else args.batch_size
    ...
    if args.ckpt == 'debug' or not args.save_ckpt:
        ...
        ckpt_callback = get_checkpoint_callback(
            ...
            every_n_train_steps=max(dm.num_samples//global_bs, 1))
    ...
    # * Accelerating *
    if args.num_gpus > 1 and (args.accelerator is None and args.plugins is None):
    ...
    if pl.utilities.distributed._get_rank() == 0:
        print('\r\ntest/OA: {}'.format(
            model.test_scores['Accuracy'].item()))
        print('test/Mean_IoU_by_TM: {}'.format(
            model.test_scores['JaccardIndex']))
...
if __name__ == '__main__':
    ...
    # Data Info
    parser.add_argument('--dataset', default='dlrsd', type=str)
    parser.add_argument('--data_seed', default=0, type=int)
    parser.add_argument('--train_init_data', type=str,
                        default=None)

    parser.add_argument('--train_scale', default=0.05, type=float,
    ...
    parser.add_argument('--model_name', default='101'+'2300'+'002', type=str)
    ...
    if type(args.gpus) is list:
        args.num_gpus = len(args.gpus)
    else:
        args.num_gpus = args.gpus

    # print(args.map_keys)

    # checkpoint_standardize('/home/taopc/Data/Data_Lib/JiQi/Model/SSD/0102300000_21200002912_21210131850_finetune')
    main(args, stream)
    stream.close()
